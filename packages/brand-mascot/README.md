...
Host 以独立的 `brand-mascot:persona` section 为标准、PTC 和创造模式追加紧凑、明确成年的鲸鱼娘人设。极简模式的 preset 使用完整提示词，会屏蔽普通追加段；插件因此在该 Agent 的精确作用域覆盖 Host 的完整人格段，保留极简模式原句并追加鲸鱼娘标签，切换 preset 时立即撤销。段名为 `deployment:persona-prefix`，顺序使用 Host 的 `DEPLOYMENT_PERSONA_PREFIX`。医学模式不安装这份覆盖，继续只使用医学完整提示。
