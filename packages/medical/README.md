...
| 第五项“医学模式” | 直接粘贴零散病史、检查单转写、用药记录、会诊意见和互相矛盾的备注；Fable 自行抽取事实、时间线、重复项和冲突 | 会话持续使用医学完整提示与 Fable；每条用户消息最多触发一个模型步骤，不暴露或执行工具 |
...
2. 用户选择医学模式时，DSH 通过标准 `agent-preset/selected` 事件把选择写入会话日志；插件只对当前解析结果为 `medical` 的 Agent 写入 Fable 请求头。
3. Preset 装入完整医学系统提示、抑制运行时编码上下文、限制全部工具，并拒绝同一用户消息产生第二个模型步骤。
4. 用户消息不做本地重写。提示要求 Fable 自行整理时间线、合并重复信息、并列矛盾描述，并把无法确定的字段列为待补充。
...
真实验收覆盖：默认关闭、设置启用、结构化面板、第五模式菜单、全新会话、确定性标题、标准用户消息、精确 Fable 路由、单消息单步骤、无工具、关闭后拒绝新任务、已开始任务继续完成、普通模式不受影响、结构化首条图片和医学模式会话图片，以及无重复命令、DOM、字典或插槽注册。
