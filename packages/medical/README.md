...
| 入口 | 用途 | 会话与模型行为 |
|---|---|---|
| 侧栏“医学分析” | 结构化录入年龄、性别、主诉、现病史、既往史、用药、过敏史、查体、检查、危险因素和分析重点；可在首条病例中同时选择图片 | 每次创建新的 `standard` 会话，先写确定性标题，再临时装入医学作用域，只让下一条病例消息调用一次配置的医学模型；结束后恢复原会话路由 |
...
7. 纯文本病例恢复提交前路由；含图片病例保留医学请求头，避免把已有图像历史切回文本模型。
...
路由可在设置 → 通用中修改。插件不设置 `maxTokens`、温度或重试次数，适配器和部署已有的上限继续生效。使用图片时，部署必须把该模型的输入能力声明为 `text` 与 `image`；`cc-api / claude-fable-5` 是本仓库完成真实视觉验证的示例。
...
真实验收覆盖：默认关闭、路由配置、设置启用、结构化面板、第五模式菜单、全新会话、确定性标题、标准用户消息、缓存稳定的医学路由、医学模式连续多轮、无工具、关闭后拒绝新任务、已开始任务继续完成、普通模式不受影响、结构化首条图片和医学模式会话图片，以及无重复命令、DOM、字典或插槽注册。
