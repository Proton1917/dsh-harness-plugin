...
- The brand persona is a distinct order-10 prompt section for ordinary non-complete presets. Minimal mode requires an exact-Agent `deployment:persona` shadow containing its original complete prompt plus the compact tags; retract that shadow on preset change or Agent disposal. Medical mode never receives the shadow and keeps its complete medical prompt. Keep both paths in Host source rather than a machine-local profile override.
