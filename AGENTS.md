...
- Medical analysis is disabled by default. Disabling it rejects new structured submissions and new Medical-mode requests but never aborts an admitted turn. Every structured case creates a fresh `standard` DSH session, arms medical admission before assigning its deterministic title, records the case as a standard `user/message`, and uses `cc-api` / `claude-fable-5` / `high` for one request by default. Failed admission never renames or opens the new session, and an Agent Preset selection cancels a pending structured scope before the selected preset takes ownership.
...
- Every medical request uses a complete medical system prompt, suppresses runtime context, exposes no tool schemas, and denies tool execution. Structured-case submissions reject a second model step and unwind when the Agent returns to idle. Medical mode adds no step or turn limiter: it keeps the same Fable route across user turns so conversation history and provider cache prefixes remain reusable while the Agent stays composed on the `medical` preset.
