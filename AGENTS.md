...
- `packages/live-stats` owns the replayable live token projection, DeepSeek tokenizer, latency/token row, TPS row, locale namespace, and their tests. Cache-hit percentage belongs in the built-in usage panel.
...
- `packages/brand-mascot` owns `mascot.webp`, the `sidebar.brand.mark` / `sidebar.brand.name` occupants, mascot-card and portrait-mark CSS treatment, the additive `brand-mascot:persona` system-prompt section, and their tests.
- `packages/medical` owns medical admission settings, the `medical` Agent Preset source and sync script, persistent cache-stable routing, case validation and prompt rendering, the sidebar structured-case desk, the General settings row, and their tests.
...
- The explicitly adult brand persona is a distinct order-10 prompt section for ordinary non-complete presets. Minimal mode requires an exact-Agent shadow of the active Host's persona section (`deployment:persona-prefix`), using the Host's placement and containing its original complete prompt plus the compact tags; retract it on preset change or Agent disposal. Medical mode never receives the shadow and keeps its complete medical prompt. Keep both paths in Host source rather than a machine-local profile override.
- Host event handlers classify a target Agent through the owning plugin's injected `ctx.agentPresets.composedPreset(agent.ctx)`. Never read `agent.ctx.agentPresets`: the Agent context does not own the roster injection, and throwing from a synchronous `agent/created` listener vetoes every new Session.
...
- Live token counts may estimate in-progress usage, but final provider usage replaces the estimate. TPS prefers real streamed output deltas and arrival time; between continuous samples, fall back to DSH's completed-step decode tokens divided by their decode spans rather than hiding the row.
- Medical analysis is disabled by default. Disabling it rejects new Medical-mode requests but never aborts a request already inside the Agent Loop. Every structured case creates a fresh `medical` DSH session, assigns its deterministic title before the Prompt, records the case as a standard `user/message`, and uses the Anthropic-locked OpenRouter route `anthropic` / `anthropic/claude-fable-5.1` / `high` by default. Rename or Prompt failure never opens the new session.
...
- Every medical request uses a complete medical system prompt, suppresses runtime context, exposes no tool schemas, and denies tool execution. The structured launcher and direct unorganized input both use the real `medical` Agent Preset. Medical mode adds no step or turn limiter: it keeps the same route across user turns so conversation history and provider cache prefixes remain reusable while the Agent stays composed on the preset.
- Medical routing is Session-local. Record route intent with `model/selection`; only the Agent Loop writes turn-enclosed `request/header` snapshots. The model selector reads the Host projection. The plugin never calls `session.selectModel`, because that user action also persists the selected route as the global Agent default.
...
- Install and remove the fifth mode with `pnpm run medical:preset:install` and `pnpm run medical:preset:remove`. The scripts generate a private local declaration bundle in the managed directory and install/remove it through the official Profile CLI. They may update or delete only a directory carrying their management marker; an unmanaged same-name preset is never overwritten or removed.
...
- The current source compatibility assembly and every exact dev dependency target DSH `0.1.7-alpha.1`; package peer ranges target the same prerelease line. Client plugins use Cordis `Context` directly, Session and Workspace faces from their API Controller packages, configuration forms from `dsh-client-ui-settings`, and slot declarations from `dsh-client-ui-renderer`; never restore the removed aggregate `dsh-client-runtime`. Durable projections keep `stateSchema` plus an explicit Client `wire`; in-flight statistics consume Session Controller transient `assistant/live-chunk` entries.
