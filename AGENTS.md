...
- The current source compatibility assembly and every exact dev dependency target DSH `0.1.6-alpha.1`; package peer ranges target the same prerelease line. Client plugins use Cordis `Context` directly, Session and Workspace faces from their API Controller packages, settings scopes from `dsh-client-ui-settings`, and slot declarations from `dsh-client-ui-renderer`; never restore the removed aggregate `dsh-client-runtime`. Durable projections keep `stateSchema` plus an explicit Client `wire`; in-flight statistics consume Session Controller transient `assistant/live-chunk` entries.
